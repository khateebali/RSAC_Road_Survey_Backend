# RSAC Road Survey Backend

Spring Boot backend for RSAC-UP's multi-city road inventory survey platform. **Ghaziabad Nagar
Nigam (GNN)** is the first onboarded city (Tender Ref: 129/Nirman/2026-27) — every table carries
`nagar_nigam_id` so onboarding another Nagar Nigam is data, not a schema change.

Own database (`gnn_road_survey` on `db-primary` — kept as-is, renaming a live database is not
worth the churn), own MinIO object store (`gnn-survey-minio` on `staging-db`, port 9002/9003),
deployed standalone on `staging-db` port 8070 — fully independent of the existing `LKO_NN`
backend.
